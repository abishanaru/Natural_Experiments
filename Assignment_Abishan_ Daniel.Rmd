...
### Abishan
```{r}
compute_streaks <- function(results) {
  n <- length(results)
  win_streak <- numeric(n)
  lose_streak <- numeric(n)
  
  win_count <- 0
  lose_count <- 0
  
  for (i in seq_along(results)) {
    # Record streaks before the current match:
    win_streak[i] <- win_count
    lose_streak[i] <- lose_count
    
    # Update counts based on current match result:
    if(results[i] == "W"){
      ...
      win_count <- 0
    } else {  # Assuming draws reset both streaks:
      win_count <- 0
      lose_count <- 0
    }
  }
  return(data.frame(win_streak_before = win_streak,
                    lose_streak_before = lose_streak))
}

# Compute streaks for your team:
streaks_team <- compute_streaks(premier_league_2024$Result)
premier_league_2024 <- bind_cols(premier_league_2024, streaks_team)

# Compute opponent's streaks (if desired) using their results:
streaks_opp <- compute_streaks(premier_league_2024$opponent_result)
premier_league_2024 <- premier_league_2024 %>%
  rename(win_streak_opp = win_streak_before,
         lose_streak_opp = lose_streak_before) %>%
  bind_cols(streaks_opp)
...
# First, add the opponent's result based on your team's result:
premier_league_2024 <- premier_league_2024 %>%
  mutate(opponent_result = case_when(
    Result == "W" ~ "L",
    Result == "L" ~ "W",
    TRUE         ~ "D"
  ))

# Pivot the dataset so that each match appears twice.
# For your team ("Team") and the opponent ("Opponent"), assign the correct result and venue.
premier_long <- premier_league_2024 %>%
  pivot_longer(
  ...
  mutate(
    # If the row comes from the Team column, the result is as recorded.
    # If it comes from the Opponent column, use the computed opponent_result.
    result = if_else(side == "Team", Result, opponent_result),
    # Switch venue for the opponent: if your team is Home, then for the opponent it’s Away, and vice versa.
    venue = if_else(side == "Team", Venue, if_else(Venue == "Home", "Away", "Home"))
  ) %>%
  arrange(club, Date)

# Now, for each club, compute streaks over time:
premier_long <- premier_long %>%
  group_by(club) %>%
  arrange(Date) %>%
  mutate(
    streaks = list(compute_streaks(result))
  ) %>%
  unnest(cols = c(streaks), names_sep = "_")%>%
  ungroup()
...
```{r}
model_long <- glm(
  as.numeric(result == "W") ~ streaks_win_streak_before + streaks_lose_streak_before + venue,
  data = premier_long,
